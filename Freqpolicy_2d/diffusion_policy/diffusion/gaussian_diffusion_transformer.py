...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            if model_output.shape != (B, C ,action_dim*2, *x.shape[3:]):
                import pdb; pdb.set_trace()
            assert model_output.shape == (B, C ,action_dim*2, *x.shape[3:])
            model_output, model_var_values = th.split(model_output, action_dim, dim=2)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                B, C ,action_dim= x_t.shape[:3]
                assert model_output.shape == (B, C ,action_dim*2, *x_t.shape[3:])
                model_output, model_var_values = th.split(model_output, action_dim, dim=2)
