...
from diffusion_policy.model.far.diffloss import DiffLoss
...
class FAR(nn.Module):
    ...
    def sample_tokens_nomask(self, bsz, num_iter=5, conditions=None, cfg=3.0, temperature=1.0):
        # """使用渐进式掩码的轨迹生成过程"""
        # # 获取设备信息
        # device = conditions.device
        # dtype = conditions.dtype  # 直接从conditions获取数据类型
        ...
        #     cur_tokens = torch.zeros_like(tokens)
        #     # 编码器-解码器前向传播
        #     x = self.forward_mae_encoder(tokens, condition_embedding, index=current_freq_idx)
        #     z = self.forward_mae_decoder(x, condition_embedding, index=current_freq_idx)
        ...
        for step in steps:
            ...
            current_steps = None
            # # 计算当前迭代的采样步数，从1开始，最后一次为10，中间均匀分布
            # current_steps = int(2 + (10 - 2) * step / (num_iter - 1)) if num_iter > 1 else 10
            # if steps[-1] == step:
            #     current_steps = 10
            # else:
            #     current_steps = 10
