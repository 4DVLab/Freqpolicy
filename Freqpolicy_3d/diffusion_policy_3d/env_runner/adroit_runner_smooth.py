...
class AdroitRunner(BaseRunner):
    ...
    def run(self, policy: BasePolicy):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Adroit {self.task_name} Pointcloud Env",
            ...
            # 创建保存目录
            save_dir = f"/inspurfs/group/mayuexin/zym/AR_model/DCT_Frequency_action/smooth_result/dp3/{self.task_name}"
            ...
            while not done:
                ...
                # 保存动作
                episode_actions.append(action.copy())  # 复制避免引用问题
            ...
            filename = os.path.join(save_dir, f"episode_{episode_idx}_actions.npy")
